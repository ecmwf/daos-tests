...
# check backend
if [ -z "$backend" ] ; then
	echo "Either --daos, --rados or --posix must be specified."
	exit 1
fi
...
test_src_dir=$HOME/daos-tests/google/ceph/fdb_hammer
...
if [[ "$backend" == "daos" ]] ; then
    ...
    cp $test_src_dir/schema_rados $tmp_dir/schema
    cp $test_src_dir/config_rados.yaml.in $tmp_dir/config.yaml
    sed -i -e "s#@SCHEMA_PATH@#${tmp_dir}/schema#" $tmp_dir/config.yaml
    sed -i -e "s#@POOL@#${pool_id}#" $tmp_dir/config.yaml
else
