...
if [ ! -e ior/test_wrapper.sh ] ; then
	echo "./ior/test_wrapper.sh not found. Please cd to daos-tests/google/ceph"
fi
...
script_filename="runs/ceph_$arg_string".sh

cat > $script_filename <<EOF
#!/bin/bash

#SBATCH --job-name="ceph_$arg_string"
#SBATCH --output=runs/ceph_${arg_string}.out
#SBATCH --error=runs/ceph_${arg_string}.err
