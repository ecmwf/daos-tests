...
# USAGE: ./benchio/submitter.sh <N_CLIENT_NODES> <SCRIPT_NAME> <IOR_API> <FABRIC_PROVIDER> <SCRIPT_ARGS>

if [ ! -e benchio/test_wrapper.sh ] ; then
	echo "./benchio/test_wrapper.sh not found. Please cd to daos-tests/google/daos"
fi
...
script_filename="runs/benchio_$arg_string".sh

cat > $script_filename <<EOF
#!/bin/bash

#SBATCH --job-name="bio_$arg_string"
#SBATCH --output=runs/benchio_${arg_string}.out
#SBATCH --error=runs/benchio_${arg_string}.err

#SBATCH --cpus-per-task=32
#SBATCH --time=00:10:00

srun benchio/test_wrapper.sh ${forward_args[@]}
EOF
