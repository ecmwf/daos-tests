	...
	while [ $out -gt 0 ] ; do
	  ...
	  out=$(squeue | grep ${USER::8} | wc -l)
        ...
        if [ $res -eq $nnodes ] ; then
          ...
          #  sleep 3
          #  sinfo -N | tail -n+2 | grep -e 'mix#' -e 'idle#' -e 'drain' | awk '{print $1}' | xargs -I{} sudo su - slurm -c 'scontrol update nodename={} state=POWER_DOWN'
	  #  out=$(sinfo -N | tail -n+2 | grep -v -e 'idle ' -e 'idle~' | wc -l)
	  #  while [ $out -ne 0 ] ; do
          #    echo "Waiting for all nodes to be idle or idle~..." && sleep 20
	  #    out=$(sinfo -N | tail -n+2 | grep -v -e 'idle ' -e 'idle~' | wc -l)
	  #  done
	  #  retried=1
	  #else
